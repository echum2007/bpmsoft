...
## UId пакета Custom (legacy — запланирована чистка, не срочно)
...
Custom зависит от CTI → его замещения применяются ПОВЕРХ CTI. Схемы попали туда по ошибке системы, не намеренно. Новые доработки в Custom не вносить — только в CTI. Чистка (перенос в CTI + удаление дублей из Custom) запланирована, но не является приоритетом первой очереди.
...
**Поля для хранения CC:** текстовые (`varchar 500`). Формат хранения — `"; "` (точка с запятой + пробел), устанавливается `MergeAddresses` в `UsrCcAddressResolver`; C#-парсинг читает по `{ ' ', ';' }`. JS-валидатор на странице принимает при вводе пробел, `;` и `,` (`/[\s;,]+/`) — это разные слои: хранение vs. ввод пользователя.
