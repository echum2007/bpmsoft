...
**⚠️ Важно:** Пространства имён задаются в свойствах процесса → вкладка **«Методы»** → блок **«Usings»**. По умолчанию список **пустой** — без добавления `BPMSoft.Configuration` ScriptTask не найдёт пользовательские C# классы и выдаст `CS0246` при публикации. Добавлять **до** первой публикации процесса.
...
Для фильтрации входящих писем от клиента в StartSignal на объекте Activity:

- ✅ **Правильно:** `Тип сообщения = Входящее`
- ❌ **Нет такого поля:** «Направление» — поля с таким именем нет в фильтрах Activity

Полный набор фильтров для «входящий email по обращению»:
- `Тип = Email`
- `Тип сообщения = Входящее`
- `Обращение заполнено`
- `Обработано процессами сервиса = Нет` (ServiceProcessed=false)

### Имя элемента StartSignal

Имя элемента (по умолчанию `StartSignal1`) используется в коде ScriptTask для получения RecordId:

```csharp
notifier.ActivityId = Get<Guid>("StartSignal1.RecordId");
```

Если имя отличается — смотреть в правой панели свойств элемента → поле **«Имя»**.
